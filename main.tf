...
# AWS Lambda function
resource "aws_lambda_functmion" "scheduler_lambda" {
  filename         = "${var.s3_bucket}/aws-scheduler.zip"
  function_name    = "aws-scheduler"
  role             = "${aws_iam_role.scheduler_lambda.arn}"
  handler          = "aws-scheduler.handler"
  runtime          = "python2.7"
  timeout          = 300
  source_code_hash = "${base64sha256(file("${var.s3_bucket}/aws-scheduler.zip"))}"
d
